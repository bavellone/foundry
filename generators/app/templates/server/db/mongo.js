...
import {saveCB} from '../libs/errors';
...
function list(model, hook) {
	...
	return function list(req, res, next) {
		...
		hook.pre(req, res, function (err, ops) {
			...
			model.findAll(ops || {}, saveCB(next, (docs) => {
...
function read(model, hook) {
	...
	return function read(req, res, next) {
		...
		hook.pre(req, res, function (err) {
			...
			model.read(req.params.id, saveCB(next, function (doc) {
...
function create(model, hook) {
	return function create(req, res, next) {
		...
		hook.pre(req, res, function (err) {
			...
			// Save the new model
			model.save(req.body, saveCB(next, function (doc) {
				hook.post(doc, req, res, function (err) {
					if (err)
						return next(err);
...
function update(model, hook) {
	return function update(req, res, next) {
		...
		hook.pre(req, res, function (err) {
			...
			model.read(req.params.id, saveCB(res, function (doc) {
				// Update the model
				...
				model.save(doc, saveCB(next, function () {
					hook.post(doc, req, res, function (err) {
						if (err)
							return next(err);
...
function destroy(model, hook) {
	return function destroy(req, res, next) {
		...
		hook.pre(req, res, function (err) {
			...
			model.findById(req.params.id, saveCB(next, function (doc) {
				model.remove({_id: req.params.id}, saveCB(next, function () {
